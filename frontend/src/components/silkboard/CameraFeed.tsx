...
// ─── Camera Image Resolver (6 Unique High-Res CCTV Perspectives) ────────────
...
// ─── Camera-Specific AI Optical Bounding Boxes ──────────────────────────────
...
// ─── Real-Time Millisecond CCTV Timestamp Hook ──────────────────────────────
  ...
  return (
    <div className={`camera-feed-frame ${isAlerting ? "is-alert" : ""} ${status === "offline" ? "is-offline" : ""}`}>
      <div className="camera-viewport">
        {/* Photorealistic Camera Stream Still */}
        {status === "offline" ? (
        ...
        {/* Dynamic rain streak simulation canvas */}
        {status !== "offline" && <RainOverlayCanvas isRaining={isRaining} />}

        {/* Authentic CRT scanlines & vignetting */}
        <div className="camera-scanlines" />

        {/* Multiple YOLO Optical Bounding Boxes */}
        {status !== "offline" &&
          boxes.map((box) => (
...
// ─── Camera Detail Modal with 3-Camera Switcher & Optical Analytics ─────────
...
export function CameraDetailModal({
  ...
  // Find nearby road sensor telemetry
    ...
    <>
      <div className="camera-modal-backdrop" onClick={onClose}>
        <div className="camera-modal" onClick={(e) => e.stopPropagation()}>
          ...
          {/* Quick Multi-Camera Switcher Bar */}
              ...
              <div className="camera-info-section optical-analytics-card">
                ...
                {cameraState.detection_active ? (
                  ...
                  <div className="camera-clear-note">
                    <p>✓ No visual obstructions detected. Drain grates and roadway corridors are optical clear.</p>
                  </div>
                )}
              </div>

              {/* Corroborating Surface Sensors */}
