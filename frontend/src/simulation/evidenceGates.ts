...
export function evaluateHeuristicCausal(
  ...
  // Signal 3: Upstream rainfall explanation
...
// ─── Optical Visual Triage Simulation ───────────────────────────────────────
...
export async function evaluateGate1(
  ...
  const targetCorruptDrain = ctx.snapshot.config.blocked_drain_id || "BLR-SKB-103";
  const isCorrupt = Boolean(
    activeFailures?.has("sensor_corrupt") &&
      (drain.drain_id === "BLR-SKB-103" || drain.drain_id === targetCorruptDrain),
  );
  ...
  // Evidence 1: Sensor Signal Integrity
  ...
  // If corrupt, trigger self-healing sensor isolation
  if (!isSignalValid) {
    wasHealed = true;
    effectiveWaterLevel = 92; // Secondary estimate from inlet telemetry
    recovery = {
      type: "fallback",
      description: `Sensor ${drain.drain_id} output corrupted (flatlined at 0.0 cm) — Initiating sensor isolation & falling back to adjacent street inlet INL-02 telemetry`,
      outcome: "Inlet INL-02 reports severe surface backpressure (88 L/s). Anomaly confirmed via secondary sensor grid.",
      executed: true,
    };
  }

  // Evidence 2: Water level threshold
  ...
  // Evidence 3: Flow velocity anomaly
  ...
  // Evidence 5: 15-Second Continuous Recording Chunk
...
export async function evaluateGate2(
  ...
  // Quota Safeguard: If Gate 1 found no anomaly, skip Gemini API entirely
  if (!gate1Passed) {
    judgment = {
      drain_id: drain.drain_id,
    ...
    // Handle Gemini Timeout failure or live call
    wasHealed = true;
    evidence.push({
      id: "g2-e1",
      source: "gemini:flash",
    ...
    recovery = {
      type: "fallback",
      description: "Gemini Flash Live API timeout (>4000ms) — Triggering circuit breaker and falling back to L3 Core Deterministic Heuristic Engine",
    ...
    // Force Hallucination: Gemini claims normal runoff despite high water level!
    judgment = {
      drain_id: drain.drain_id,
      classification: "normal_runoff",
    ...
    // Live Gemini Call with 4-second timeout circuit breaker and strict 15s rate limiter
    try {
      ...
      // Circuit breaker fallback to heuristic
      wasHealed = true;
      judgment = evaluateHeuristicCausal(drain, neighbors, historyWaterLevels);
      recovery = {
        type: "fallback",
        description: "Gemini API unavailable or rate-limited (>15s cooldown) — Fallback to L3 Core Deterministic Heuristic Engine",
  ...
  // Evidence 2: Rainfall conservation model
...
export async function evaluateGate3(
  ...
  const isCameraOffline = Boolean(activeFailures?.has("camera_offline") && primaryCam.id === "CAM-02");
  ...
  // Evidence 1: Primary camera status
  evidence.push({
    id: "g3-e1",
  ...
  if (isCameraOffline) {
    wasHealed = true;
    // Reroute to secondary camera CAM-01 (North Approach)
    const backupCam = CAMERAS.find((c) => c.id !== primaryCam.id) || CAMERAS[0];
    cameraUsed = backupCam.id;

    recovery = {
      type: "re-plan",
      description: `Primary CCTV camera ${primaryCam.id} is OFFLINE — Causal re-planning: Rerouting visual triage to adjacent camera ${backupCam.id} (${backupCam.label})`,
  ...
  // Evidence 3: Optical frame clarity
  evidence.push({
    id: "g3-e3",
...
// ─── Gate 4: Cross-Validation (The Zero-Trust Gate) ──────────────────────────
...
export async function evaluateGate4(
  ...
  // Zero-Trust Check: Does LLM reasoning contradict physical telemetry + camera?
  if (isHallucination) {
    wasHealed = true;
  ...
  // Evidence 2: Street ponding correlation from road sensors
  ...
  // Evidence 3: Historical pattern confidence
  evidence.push({
    id: "g4-e3",
...
export async function evaluateGate5(
  ...
  // Evidence 1: WhatsApp Primary Channel ACK
  if (isDispatchNoAck) {
    wasHealed = true;
    evidence.push({
      id: "g5-e1",
  ...
  // Evidence 2: Dispatch payload JSON schema validation
  evidence.push({
    id: "g5-e3",
...
export async function executeEvidenceGatedPipeline(
  ...
  // ── Step 1: Gate 1 (Anomaly Detection)
  ...
  // ── Step 2: Gate 2 (Causal Disambiguation)
  ...
  // ── Step 3: Gate 3 (Visual Verification)
  ...
  // ── Step 4: Gate 4 (Cross-Validation)
  ...
  // ── Step 5: Gate 5 (Dispatch Execution)
