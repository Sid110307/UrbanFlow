...
function generateDrainTelemetry(
  ...
  return DRAIN_NODES.map((node, i) => {
    ...
    const corruptTargetDrain = config.blocked_drain_id || DEFAULT_SENSOR_CORRUPT_DRAIN_ID;
    if (activeFailures?.has("sensor_corrupt") && node.drain_id === corruptTargetDrain) {
      waterLevel = 0;
      flowVelocity = 0;
      turbidity = 0;
    }
...
export function useSilkboardSimulation() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const elapsed = tick * (TICK_INTERVAL_MS / 1000) * configRef.current.speed;

      const currentConfig = { ...configRef.current, elapsed_seconds: elapsed };
  ...
  const startScenario = useCallback((scenario: SilkboardScenarioKind, blockedId?: string | null) => {
    ...
    setConfig((c) => ({
      ...
      blocked_drain_id: scenario === "blockage" ? (blockedId ?? DEFAULT_SENSOR_CORRUPT_DRAIN_ID) : null,
      elapsed_seconds: 0,
      running: true,
    }));
    setDrainHistory({});
